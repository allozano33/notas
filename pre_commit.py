...
def main():
    ...
    try:
        ...
        exit_code = 3
        logger.error("[repo:%s][check_skipped:%s][exit_code:%s][uuid:%s][error:%s][full_error:%s]",
                repo, check_skipped, exit_code, uuid, err, err.child_exception)
        print(f"{red}There was an error getting the diff. Try again and contact websec@ or #tech-websec if the error persists.{reset}")
